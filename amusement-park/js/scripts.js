...
window.onload = function () {
	document.querySelector("form").onsubmit = function (event) {
		...
		document.getElementById("clickMe").onclick = hideResultsAndError;
		
		const el = document.getElementById("clickMe");
		if (el.addEventListener)
			el.addEventListener("click", hideResultsAndError, false);
		else if (el.attachEvent)
			el.attachEvent('onclick', hideResultsAndError);
		// if (button === reset) {
		// 	hideResultsAndError();
		// }
